    ...
    if (req.body != null) {
      if (req.body.cpass == req.user.password) {
        if (req.body.cpass != req.body.npass) {
          if (
            passwordRegex.test(req.body.npass) &&
            req.body.npass.length >= 6 &&
            req.body.npass.length <= 16
          ) {
            ...
            req.flash(
              "error",
              "please enter a strong password (6-16 characters)"
            );
            return res.redirect("back");
          }
        } else {
    ...
    if (req.body != null) {
      if (
        ...
        if (req.body.npass == req.body.cpass) {
          ...
          if (checkemail) {
            let changepass = await user.findByIdAndUpdate(checkemail.id, {
              password: req.body.npass,
            });
            if (changepass) {
              res.clearCookie("otp");
            ...
            req.flash(
              "error",
              "email id not found, please enter right email address"
            );
            return res.redirect("back");
          }
        } else {
...
module.exports.fuel = async (req, res) => {
  try {
    ...
    let totaldata = await fuel
      .find({
        $or: [{ landmark: { $regex: search, $options: "i" } }],
      })
      .countDocuments();
            ...
            if (req.body.gender !== undefined) {
              if (req.file) {
                ...
                if (single) {
                  ...
                  req.body.name = req.body.fname + " " + req.body.lname;
                  let udata = await user.findByIdAndUpdate(
                    req.params.id,
                    req.body
                  );
                  if (udata) {
  ...
  try {
    if (req.body) {
      const session = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url:
          "http://localhost:8080/user/fuelpayment/success?session_id={CHECKOUT_SESSION_ID}",
...
module.exports.paymentfuelSuccess = async (req, res) => {
  ...
  let data = await forder.findOneAndUpdate(
...
module.exports.paymentmechSuccess = async (req, res) => {
  const { session_id } = req.query;
  let data = await morder.findOneAndUpdate(
    { session_id: session_id },
    { payment_status: "paid" }
  );
  await data.save();
    ...
    if (req.body) {
      if (addressRegex.test(req.body.address)) {
        ...
        const session = await stripe.checkout.sessions.create({
          ...
          mode: "payment",
          success_url:
            "http://localhost:8080/user/mechpayment/success?session_id={CHECKOUT_SESSION_ID}",
...
module.exports.cancel = async (req, res) => {
  try {
    ...
    if (fueldata) {
      let cancel = await forder.findByIdAndUpdate(req.params.id, {
        status: "cancel",
      });
      if (cancel) {
      ...
      let cancel = await morder.findByIdAndUpdate(req.params.id, {
        status: "cancel",
      });
      if (cancel) {
        req.flash("success", "order canceled");
